...
function DashBoardScreen() {
  ...
  const [reminder_id, setReminderID] = useState("");
      ...
      const getListings = async () => {
        try {
          ...
          if (res.status == 200) {
            ...
            /*const objectId = data.my_listings.map((listing) => listing._id);
            const objectIdArrayString = JSON.stringify(objectId);
            setID(objectIdArrayString);
            console.log(objectIdArrayString);
  ...
  const deleteListing = async (list_id) => {
    setListingID(list_id);
    try {
  ...
  const deleteReminder = async (remind_id) => {
    setReminderID(remind_id);
  ...
  return (
    <SafeAreaView style={styles.background}>
      ...
      {reminderPopPressed && popupVisible && (
        <ReminderPopup reminder={selectedItem} hidePopup={setPopupVisible} />
      )}
      {listPopPressed && popupVisible && (
