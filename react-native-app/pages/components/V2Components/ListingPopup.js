  ...
  return (
    <BlurredPopup onExitPress={() => hidePopup(false)}>
      ...
      >
        ...
        <View style={{ paddingHorizontal: 20 }}>
          <Text
            style={[
              styles.text,
              { fontWeight: "bold", fontSize: 25, paddingBottom: 2 },
            ]}
          >
            {`${listing.city}, ${listing.zip_code}`}
          </Text>
          <Text style={[styles.text, { fontSize: 16, paddingBottom: 2 }]}>
            {listing.street_name}
          </Text>
          <Text style={[styles.text, { fontSize: 16, paddingBottom: 2 }]}>
            {listing.rent}
          ...
          <View style={{ flexGrow: 1, flexDirection: "row" }}>
            ...
            >
              <View style={{ flexDirection: "row" }}>
                ...
                <View>
                  {listing.gender === "male" && (
                    <Ionicons name={"male-outline"} size={16} color={"blue"} />
                  )}
      ...
      <View style={{ height: "34%", paddingTop: 5, paddingLeft: 8 }}>
        <Text style={{ color: theme.TEXT_COLOR }}>{listing.bio}</Text>
      </View>
      <Text
        style={{
      ...
      >
        Last updated:{" "}
        {Math.floor(
          (Date.now() - Date.parse(listing.updatedAt)) / (1000 * 60 * 60 * 24)
        )}{" "}
        days ago
      </Text>
    </BlurredPopup>
